...
function BillItem({ title, asset, description, amount }: BillItem) {
  return (
    <div className={s.bill_item}>
      <div className={s.bill_content}>
        <h5 className={s.bill_item_title}>{title}</h5>
        {description && (
          <p className={s.bill_item_description}>{description}</p>
        )}
      ...
      <div className={s.bill_container}>
        ...
        <div className={s.bill_items}>
          ...
          <BillItem
            ...
            description={`${cancelledSubs?.length || 0} subscriptions`}
            amount={cancelledSum || 0}
          />
        </div>

        <div className={s.bill_new}>
          <div className={s.bill_content}>
            <h3 className={s.bill_new_title}>New Bill</h3>
            <p className={s.bill_new_asset}>
              {(primarySubs?.length || 0) - (cancelledSubs?.length || 0)}
              &nbsp;subscriptions
            </p>
          </div>
          <div className={s.bill_new_value}>=&nbsp;${newBill}</div>
        </div>
      </div>
    </SideBlock>
  );
